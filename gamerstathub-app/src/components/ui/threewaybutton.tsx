...
const ThreeWayButton: React.FC<ThreeWayButtonProps> = ({ filter, onFilterChange }) => {
  return (
    <div className="relative w-48 h-10 rounded-full flex items-center">
      ...
      ></div>
      <button
        className={`w-1/3 text-sm text-center z-10 ${filter === "all" ? "text-white" : "text-gray-200"}`}
        onClick={() => onFilterChange("all")}
      >
        All
      </button>
      <button
        className={`w-1/3 text-sm text-center z-10 ${filter === "friends" ? "text-white" : "text-gray-200"}`}
        onClick={() => onFilterChange("friends")}
      >
        Friends
      </button>
      <button
        className={`w-1/3 text-sm text-center z-10 ${filter === "none" ? "text-white" : "text-gray-200"}`}
